...
namespace Business
{
	namespace Business
	{
		public static class SettingsHandler
		{
			...
			{
				TSettingDatatype setting_datatype = BConstants.GetSettingDataTypeFromType(type);

				string sql = "";

				switch (setting_datatype)
				{
					case TSettingDatatype.String: sql = "INSERT INTO settings(name, string_value, datatype, user_id) VALUES(@name, @value, 'string', @user_id);"; break;
					case TSettingDatatype.Boolean: sql = "INSERT INTO settings(name, boolean_value, datatype, user_id) VALUES(@name, @value, 'boolean', @user_id)"; break;
					case TSettingDatatype.Numeric: sql = "INSERT INTO settings(name, numeric_value, datatype, user_id) VALUES(@name, @value, 'numeric', @user_id)"; break;
					case TSettingDatatype.Blob: sql = "INSERT INTO settings(name, blob_value, datatype, user_id) VALUES(@name, @value, 'blob', @user_id)"; break;
				}
				...
				using (var cmd = new NpgsqlCommand(sql, conn))
				{
					...
					using (var reader = cmd.ExecuteReader())
					{
						if (reader.HasRows)
						{
							...
							TSettingDatatype setting_datatype = BConstants.GetSettingDataTypeFromString(reader.GetString(5));

							switch (setting_datatype)
							{
								case TSettingDatatype.Numeric: value = numeric_value; break;
								case TSettingDatatype.Boolean: value = boolean_value; break;
								case TSettingDatatype.String: value = string_value; break;
								case TSettingDatatype.Blob: value = blob_value; break;
							}
						}
